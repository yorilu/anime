...
;define(['zepto', 'hammer'], function (zepto, Hammer){
  ...
  var anole = window.anole = {
      ...
      _init: function (){
		...
		this.muteBtn.appendTo(_container);
        var startBtn = this._startBtn =  $(this._config.startBtnTemplate);
        _container.append(startBtn);
        
		startBtn.on('click', startAnime);
	    
		var autoBtn = this._autoBtn = $('<div class="btn auto-btn">Auto Play</div>').appendTo(_container);
        ...
        if(this._config.flipType == 'click'){
          ...
          _container.append(prevBtn).append(nextBtn);
